...
class CreatorExperienceTests(unittest.TestCase):
    ...
    def test_create_page_includes_progress_ui(self) -> None:
        ...
        self.assertIn("No documents uploaded. The module will use web search if enabled", quick_create.text)
        self.assertIn("One objective is enough to generate a focused module.", quick_create.text)
        self.assertNotIn("fewer goals", quick_create.text)
        self.assertNotIn("fewer sources", quick_create.text)
        ...
        self.assertIn("Module generation took longer than expected. Please try again in a moment.", create_js)
        self.assertNotIn("Please try fewer goals or fewer sources.", create_js)
